...
/// The factory finds a top-level-statement entry point through a process-wide diagnostic
/// listener, so two factories starting at the same moment can capture each other's host and
/// a test then fails on an <c>ObjectDisposedException</c> for a host it never started.
